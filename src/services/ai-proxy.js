...
const DIRECT_RECHARGE_URL = typeof __DIRECT_RECHARGE_URL__ !== 'undefined' ? __DIRECT_RECHARGE_URL__ : '';
...
/**
 * 一步直达充值:输入金额 → 立即加积分 → 写流水。
 * 不创建 pending 订单,不经过二维码/支付页面。
 *
 * @param {{ amount: number }} opts amount 单位:分
 * @returns {{ balanceAfter, credits }}
 */
export async function directRecharge({ amount }) {
  if (!Number.isFinite(amount) || amount <= 0) {
    throw new CallAIError('INVALID_PARAM', '充值金额无效');
  }
  console.log('[directRecharge] 请求:', { url: DIRECT_RECHARGE_URL, amount: Math.floor(amount) });
  const data = await authedFetch(DIRECT_RECHARGE_URL, { amount: Math.floor(amount) });
  console.log('[directRecharge] 响应:', data);
